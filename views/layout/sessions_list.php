...
<section class="km_SessionsSection">
    <div class="container-fluid">
        ...
        <div class="km_row">
            <div class="km_col_3">
                <?php 
                $searchLocation = $this->getSessionLocations($sessions);
                do_action('before_km_session_filters', $activityTags);
                 

                if(isset($isMarketplaceList)){
                    echo ActivityHub()->engine->getView('session_filters', ['locations' => $searchLocation, 'activityTags' => $activityTags->data, 'filters' => $filters['filters'], 'tagId' => $tagId,'isMarketplaceList'=>$isMarketplaceList]);
                }else{
                    echo ActivityHub()->engine->getView('session_filters', ['locations' => $searchLocation, 'activityTags' => $activityTags->data, 'filters' => $filters['filters'], 'tagId' => $tagId]);
                }
            ...
            <div class="km_col_9 km_shadow_right km_session_list">
                ...
                <div class="km_provider_sessions">
                    <?php echo ActivityHub()->engine->getView('_sessionlist_part', ['sessions' => $sessions, 'permalink' => get_the_permalink(), 'filters' => $filters, 'tagId' => $tagId]); ?>
                </div>    
            </div>
        </div>
    </div>
