...
$layout = $this->getValue('fieldday_purchase_layout_mode', $fielddaySetting, false);
// $layout;
if ($layout == "one") {
    ?>
...
do_action('before_km_session_filters', $activityTags);
    echo fieldday()->engine->getView('session_filters', ['activityTags' => $activityTags->data, 'filters' => $filters['filters'], 'tagId' => $tagId, 'tabs' => $activityTypes->data]);
    ?>
                </div>
                <?php if ($fielddaySetting['cart_menu'] == 'defaultview') {?>
                <div class="km_col_1 km_session_list_icons">
                    <?php echo fieldday()->engine->GetCartView(); ?>
                </div>
                <?php }?>
            </div>
            ...
            <div class="km_col_12 km_shadow_right km_session_list">
                <div class="km_session_tabs_content">
                    ...
                    <div class="km_provider_sessions">
                        <?php print $this->displaySessionList($layout, $tabId, $tagId, $filters, get_the_permalink());?>
                    </div>
...
<section class="km_SessionsSection">
    ...
    ?>
            ...
            <div class="km_col_9 km_shadow_right km_session_list">
                ...
                <div class="km_provider_sessions">
                    <?php
    print fieldday()->engine->getView('layout/_sessionlist_part', ['sessions' => $sessions, 'permalink' => get_the_permalink(), 'filters' => $filters, 'tagId' => $tagId]);
